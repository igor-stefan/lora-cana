...
void gerar_endereco(char *info, int mes, int dia, int hora, int minuto, int seg);
...
void gerar_endereco(char *info, int mes, int dia, int hora, int minuto, int seg){
  ...
  for(int i = 0; i < 12; i++){
    if(i == mes+1){
      convertido = meses[i - 1];
      break;
    }
  }
  sprintf(info, "%s/%02d/%02d:%02d:%02d", convertido, dia, hora, minuto, seg);
}
    ...
    if(packet_size == sizeof(TDadosLora)) 
    {
        ...
        display.setCursor(0, OLED_LINE6);
         ...
         if(!getLocalTime(&timeinfo)){
          endereco = "/Geral/falha";
          Serial.println("Failed to obtain time");
         }
         else{
           gerar_endereco(info, timeinfo.tm_mon,timeinfo.tm_mday, timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec);
           endereco = info;
           Serial.print("INFO = ");
           Serial.println(info);
         }
         
        display.print(endereco);
        display.display();     
        Firebase.pushFloat(firebaseData, "/Temperatura/" + endereco, dados_lora.temperatura);
        Firebase.pushFloat(firebaseData, "/Umidade/" + endereco, dados_lora.umidade);
    }
}
